...
public class GroupChat extends JFrame implements ActionListener, MouseListener,
	...
	public void refreshFriendsList(String[] userList) {

		jpFl.removeAll();

		for (String user : userList) {
			jlFriend = new JLabel(user);
	...
	public void refreshReceivedMsg(BcPackage msgInfo) {
		try {
			if(msgInfo.getClusteredid().equals(userBean.getUserName())){
				styledDoc.insertString(styledDoc.getLength(),
						msgInfo.getClusteredid()+"\r\n", styledDoc.getStyle("Style08"));
				styledDoc.insertString(styledDoc.getLength(), "\t"
						+ msgInfo.getBcmessage() + "\r\n",
	...
	@Override
	public void keyPressed(KeyEvent e) {//当按住  ctrl+回车键的时候
		if (e.getModifiers() == InputEvent.CTRL_MASK
				&& e.getKeyCode() == KeyEvent.VK_ENTER) {
			MsgHandleService.coreBusinessControl.doSendMsg(jtpInChat.getText());//把内容拿过来发出去。
			jtpInChat.setText("");
		}
	}
	...
	public GroupChat() {
		...
		jlMini.addMouseListener(new MouseAdapter() {
			@Override
		...
		jlExit.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				//客户端退出方法
				MsgHandleService.NoticeLoginReal(BcPackageBuilder.BcPackage.newBuilder()
			  			.setBctype("7")
			  			.build());
